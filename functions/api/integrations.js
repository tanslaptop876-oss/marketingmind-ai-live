export async function onRequestGet(context) {
  ...
  const requirements = {
    ...
    google: ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_REDIRECT_URI', 'PROVIDER_TOKEN_ENCRYPTION_KEY', 'SUPABASE_SERVICE_ROLE_KEY'],
  ...
  const integrations = [
    ...
    { id: 'youtube', name: 'YouTube video publishing', configured: configured('GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'YOUTUBE_REDIRECT_URI', 'PROVIDER_TOKEN_ENCRYPTION_KEY'), enabled: context.env.YOUTUBE_CONNECT_ENABLED === 'true' },
  ...
  ].map(item => ({ ...item, missing: (requirements[item.id] || []).filter(key => !context.env[key]) }));
