...
export async function onRequestGet(context){
  ...
  try{
    ...
    const longTokenUrl=new URL(`https://graph.facebook.com/${version}/oauth/access_token`);longTokenUrl.search=new URLSearchParams({grant_type:'fb_exchange_token',client_id:context.env.META_APP_ID,client_secret:context.env.META_APP_SECRET,fb_exchange_token:token.access_token});
    const longTokenResponse=await fetch(longTokenUrl,{headers:{accept:'application/json'}}),longToken=await longTokenResponse.json();if(!longTokenResponse.ok||!longToken.access_token)throw new Error('long_lived_token_exchange_failed');
    const accountsResponse=await fetch(`https://graph.facebook.com/${version}/me/accounts?fields=id,name,access_token,instagram_business_account{id,username}&limit=100`,{headers:{authorization:`Bearer ${longToken.access_token}`}}),accounts=await accountsResponse.json();if(!accountsResponse.ok)throw new Error('account_discovery_failed');
