...
export async function onRequestGet(context){
  ...
  const response=await fetch(`${baseUrl(context.env)}/rest/v1/mm_provider_connections?owner_id=eq.${encodeURIComponent(session.user.id)}&select=provider,account_summary,updated_at`,{headers:apiHeaders({SUPABASE_ANON_KEY:context.env.SUPABASE_SERVICE_ROLE_KEY},context.env.SUPABASE_SERVICE_ROLE_KEY)});if(!response.ok)return json({ok:false,error:'status_unavailable'},502);
